  ...
  return (
    <div className='relative'>
      <div className='flex items-center gap-3 rounded-2xl bg-white/95 shadow-lg ring-1 ring-black/5 px-4 py-3'>
        ...
        <button className='px-4 py-2 rounded-xl bg-gradient-to-r from-orange-500 to-rose-600 text-white font-semibold' onClick={()=>{ if(results[0]) navigate(`/bike/${results[0].brandSlug}/${results[0].slug}`)}}>Search</button>
      </div>

      {results.length>0 && (
        <div className='absolute mt-2 w-full rounded-xl bg-white/95 shadow-xl ring-1 ring-black/5 overflow-hidden'>
          {results.map(r=> (
            <button key={r.id} onClick={()=>navigate(`/bike/${r.brandSlug}/${r.slug}`)} className='w-full text-left px-4 py-2 hover:bg-neutral-50 flex items-center gap-3'>
