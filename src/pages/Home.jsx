      ...
      <div className="max-w-7xl mx-auto px-4 py-8 flex gap-6">
        ...
        <main className="flex-1">
          ...
          <div className="bg-gray-900 rounded-lg p-4 mb-6 flex flex-wrap items-center gap-3">
            <select value={year} onChange={(e) => setYear(e.target.value)} className="px-3 py-2 bg-gray-800 border border-gray-700 rounded text-white">
            ...
            <select value={priceRange} onChange={(e) => setPriceRange(e.target.value)} className="px-3 py-2 bg-gray-800 border border-gray-700 rounded text-white">
            ...
            <select value={style} onChange={(e) => setStyle(e.target.value)} className="px-3 py-2 bg-gray-800 border border-gray-700 rounded text-white">
              <option value="">All Body Styles</option>
              {bodyStyles.map((s) => <option key={s} value={s}>{s}</option>)}
            </select>

            <button onClick={() => { setYear(""); setPriceRange(""); setStyle(""); }} className="ml-auto px-3 py-2 bg-gray-800 border border-gray-700 rounded">
              Clear
            </button>
          </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">
              {pageItems.map((bike) => (
                <Link
                  ...
                  className="block"
                >
                  <article className="bg-gray-900 rounded-lg p-4 hover:bg-gray-800 transition h-full flex flex-col">
                    ...
                    <div className="mt-auto">
                      ...
                      <div className="text-sm text-red-600 font-semibold mt-2">₹{new Intl.NumberFormat('en-IN').format(bike.price)}</div>
                    </div>
                  </article>
                </Link>
              ))}
          ...
          <div className="mt-6 flex justify-center items-center gap-2">
            <button onClick={() => setPage((p) => Math.max(1, p - 1))} className="px-3 py-1 rounded bg-gray-800 border border-gray-700">Prev</button>
            <div className="text-sm text-gray-300 px-3">Page {page} of {totalPages}</div>
            <button onClick={() => setPage((p) => Math.min(totalPages, p + 1))} className="px-3 py-1 rounded bg-gray-800 border border-gray-700">Next</button>
          </div>
        </main>
      </div>
    </div>
